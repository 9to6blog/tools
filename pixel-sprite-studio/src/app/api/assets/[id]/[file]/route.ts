...
export async function GET(request: Request, { params }: { params: Promise<{ id: string; file: string }> }) {
  ...
  if (!/^(original\.png|reference(?:-[1-4])?\.png|sheet\.png|frame-\d{1,3}\.png|sprites\.zip|job\.json|(?:sprite|preview)-[1-9]\d{0,3}(?:x[1-9]\d{0,3})?\.png)$/.test(file)) return new Response('Not found', { status: 404 });
  try {
