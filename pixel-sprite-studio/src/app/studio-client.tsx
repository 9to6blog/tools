...
export default function Studio() {
  ...
  const busy = working || !!active || extraWorking;
  ...
  async function run(action: 'generate' | 'upload' | 'reprocess' | 'edit') {
    ...
    try {
      ...
      if (action === 'generate') {
        headers['Content-Type'] = 'application/json'; body = JSON.stringify({ id, prompt, model, quality, settings: validated, apiKey });
      } else {
