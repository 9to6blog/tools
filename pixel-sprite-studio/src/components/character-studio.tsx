...
type Props = { active: boolean; jobs: Job[]; settings: PixelSettings; apiKey: string; setApiKey: (s: string) => void; hasKey: boolean; model: string; setModel: (s: string) => void; quality: string; setQuality: (s: string) => void; refresh: () => Promise<void>; serverBusy: boolean; onBusy: (b: boolean) => void };
...
export function CharacterStudio(p: Props) {
  ...
  const disabled = busy || reading || p.serverBusy;
  ...
  async function chooseFile(file: File) {
    ...
    try { if (!file.size || file.size > MAX_UPLOAD_BYTES || !['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) throw new Error('50MB 이하 PNG, JPEG, WebP 정면 이미지를 선택하세요.'); const bitmap = await createImageBitmap(file); const pixels = bitmap.width * bitmap.height; bitmap.close(); if (pixels > 16_777_216) throw new Error('원본은 1,677만 픽셀 이하로 선택하세요.'); setRefFile(file); setSourceId(''); }
  ...
  const requests = [...views.map(() => 1), ...rows.flatMap(row => row.directions.map(() => row.frames))].map(frames => ({ model: p.model, quality: p.quality, apiSize: motionApiSize(width, height, frames), kind: 'motion' as const, referenceCount: 1 }));
  ...
  return <div className="animation-workspace character-workspace"><div className="character-intro"><strong>정면 한 장으로, 캐릭터의 모습과 동작까지.</strong><p>1. 정면 원본 추가 → 2. 좌·우·뒷모습과 동작 선택 → 3. 재생·편집·내보내기</p></div><div className="animation-builder">
    <section className="control-panel"><div className="panel-title"><h2>1. 정면 원본</h2><span>REFERENCE</span></div><div className="animation-controls"><fieldset disabled={disabled}>
      ...
      <div className="field"><Label htmlFor="characterKey">OpenAI API 키</Label><Input id="characterKey" type="password" autoComplete="off" value={p.apiKey} placeholder={p.hasKey?'환경 변수 키 사용':'sk-…'} onChange={e=>p.setApiKey(e.target.value)}/><p className="help">현재 페이지 메모리에만 보관합니다. 생성 버튼을 누를 때 요금이 발생합니다.</p></div>
